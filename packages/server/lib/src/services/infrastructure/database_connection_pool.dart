...
class DatabaseConnectionPool {
  ...
  Future<Connection> getConnection() async {
    try {
      ...
      _connectionErrors++;
      ServerLogger.error('获取连接失败', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> releaseConnection(Connection connection) async {
    try {
      ...
      ServerLogger.error('释放连接失败', error: error, stackTrace: stackTrace);
    }
  }
  ...
  Future<bool> isHealthy() async {
    try {
      ...
      ServerLogger.error('连接池健康检查失败', error: error);
      return false;
    }
  }
  ...
  Future<void> close() async {
    try {
      ...
      ServerLogger.info('数据库连接池已关闭');
    } catch (error, stackTrace) {
      ServerLogger.error('关闭连接池失败', error: error, stackTrace: stackTrace);
    }
  }
  ...
  Future<Connection> _createConnection() async {
    try {
      ...
      ServerLogger.debug('创建数据库连接成功');
      return connection;
    } catch (error, stackTrace) {
      _connectionErrors++;
      ServerLogger.error('创建数据库连接失败', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Connection> _waitForConnection() async {
    ...
    // final logger = LoggerFactory.getLogger('DatabaseConnectionPool');
    ServerLogger.debug('等待可用连接: ${_waitingQueue.length}个请求');
  ...
  Future<bool> _isConnectionHealthy(Connection connection) async {
    try {
      ...
      ServerLogger.error('连接健康检查失败', error: error);
      return false;
    }
  }
  ...
  void _startHealthCheckTimer() {
    _healthCheckTimer = Timer.periodic(Duration(minutes: 5), (timer) async {
      try {
        ...
        ServerLogger.error('健康检查定时器错误', error: error);
      }
    });
  }
  ...
  void _startCleanupTimer() {
    _cleanupTimer = Timer.periodic(Duration(minutes: 10), (timer) async {
      try {
        ...
        ServerLogger.error('清理定时器错误', error: error);
      }
    });
  }
  ...
  Future<void> _performHealthCheck() async {
    ...
    if (unhealthyConnections.isNotEmpty) {
      ServerLogger.warning('移除不健康连接: ${unhealthyConnections.length}个');
    }
  }
  ...
  Future<void> _performCleanup() async {
    ...
    if (_availableConnections.length > _minConnections) {
      ...
      if (connectionsToRemove.isNotEmpty) {
        ServerLogger.info('清理多余连接: ${connectionsToRemove.length}个');
      }
    }
  }
}
