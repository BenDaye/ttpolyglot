...
class TranslationProviderService extends BaseService {
  ...
  TranslationProviderService({
    required DatabaseService databaseService,
        ...
        super('TranslationProviderService') {
  ...
  Future<List<Map<String, dynamic>>> getUserTranslationProviders(String userId) async {
    try {
      ...
      logError('获取用户翻译接口配置失败: $userId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>?> getTranslationProviderById(String providerId, String userId) async {
    try {
      ...
      logError('获取翻译接口配置详情失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> createTranslationProvider({
    ...
    try {
      logInfo('创建翻译接口配置: $providerType for user $userId');

      // 验证提供商类型
      if (!_isValidProviderType(providerType)) {
        throwBusiness('不支持的翻译提供商类型');
      }
      ...
      logInfo('翻译接口配置创建成功: ${provider['id']}');

      return provider;
    } catch (error, stackTrace) {
      logError('创建翻译接口配置失败', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> updateTranslationProvider({
    ...
    try {
      ...
      logInfo('翻译接口配置更新成功: $providerId');

      return updatedProvider;
    } catch (error, stackTrace) {
      logError('更新翻译接口配置失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> deleteTranslationProvider(String providerId, String userId) async {
    try {
      ...
      logInfo('翻译接口配置删除成功: $providerId');
    } catch (error, stackTrace) {
      logError('删除翻译接口配置失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> testTranslationProvider(String providerId, String userId) async {
    try {
      ...
      logError('测试翻译接口配置失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> setDefaultTranslationProvider(String providerId, String userId) async {
    try {
      logInfo('设置默认翻译接口: $providerId for user $userId');
      ...
      logError('设置默认翻译接口失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> translate({
    ...
    try {
      ...
      logError('翻译接口调用失败: $providerId', error: error, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> _callTranslationAPI(
    ...
    try {
      ...
      final responseTime = DateTime.now().difference(startTime).inMilliseconds;
      // 记录响应时间统计到日志
      logInfo('翻译API调用响应时间: $responseTime ms');
    }
  }
  ...
  Future<void> _updateProviderUsage(String providerId, [int characterCount = 0]) async {
    try {
      ...
      logError('更新提供商使用统计失败: $providerId', error: error);
    }
  }
